...
export default {
  ...
  methods: {
    initData() {
      ...
      if (!storyMenu.includes("客户端小程序-编辑")) {
        this.showEdit = false;
      }
      if (!storyMenu.includes("客户端小程序-删除")) {
        this.showDelet = false;
      }
      if (!storyMenu.includes("客户端小程序-查看")) {
        this.showMore = false;
      }
      if (!storyMenu.includes("客户端小程序-新建")) {
        this.showAdd = false;
      }
    ...
    uploadPicture() {
      this.$router.push({ path: "uploadPictures", query: { bannerType: "0" } });
    },
    //编辑
