<template>
  <div>
    ...
    >
      ...
      <el-table-column align="center" prop="uMobile" label="手机号"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    initData() {
      ...
      if (!storyMenu.includes("工程部-回收站")) {
        this.recycles = false;
      }
      if (!storyMenu.includes("工程部-新建")) {
        this.showPage = false;
      }
      if (!storyMenu.includes("工程部-修改")) {
        this.editorData = false;
      }
      if (!storyMenu.includes("工程部-删除")) {
    ...
    handleCurrentChange(val) {
      ...
      clientList(currIndex).then(res => {
        this.tableList = res.result;
        console.log("aaaaa", this.tableList);
      });
      this.currpage = val;
      console.log(`当前页: ${val}`);
    },
    ...
    recycle() {
      this.$router.push({ path: "roleManagementRecycle" });
    },
    celent_more(index, row) {
      console.log(row.uid);
      this.$router.push({
        path: "engineeringDetail",
        query: { restaurant_id: row.uid }
      });
    },
    //获取批量数据
