<template>
  <div>
    ...
    >
      ...
      <el-table-column v-if="hideRow" prop="uid" width="0"></el-table-column>
      <el-table-column align="center" prop="uRealName" label="姓名"></el-table-column>
      <el-table-column align="center" prop="uMobile" label="手机号" width="180"></el-table-column>
      <el-table-column align="center" label="性别" width="180">
        <template slot-scope="scope">
          <p>{{ scope.row.uSex == '0'? '女' : '男' }}</p>
        </template>
      </el-table-column>
      <el-table-column align="center" prop="projectCount" label="项目数量"></el-table-column>
    ...
    align="center"
      ...
      <el-table-column align="center" label="操作">
        ...
        v-show="showEdit"
          size="mini"
          @click="celent_more(scope.$index, scope.row)">工程部详情</el-button>-->
          <el-button
            type="danger"
            size="mini"
            plain
    ...
    <el-dialog title="提示" :visible.sync="DeletTip" width="30%">
      <span>是否确认恢复？ 恢复信息后可到“工程部列表”查看并编辑被恢复的信息</span>
...
export default {
  ...
  methods: {
    initData() {
      ...
      if (!storyMenu.includes("工程部-工程部详情")) {
        this.showEdit = false;
      }
    ...
    testPhone() {
      ...
      if (
        this.formData.uMobile == null ||
        this.formData.uMobile.length < 11 ||
        this.formData.uMobile.test(TEL_REGEXP)
      ) {
    ...
    //   console.log(row.uid)
    //    this.$router.push({ path: 'engineeringDetail', query: { restaurant_id: row.uid }})
