...
export default {
  ...
  activated() {
    this.clientid2Name = this.$store.state.nickNameTwo;
  },
  computed: {
    ...mapGetters(["getNIckNameTwo"])
  },
  watch: {
    getNIckNameTwo: function(li) {
      //li就是改变后的wifiList值
      this.clientid2Name = li;
      console.log("改变", li);
    }
  },
  methods: {
    ...
    celent_more(index, row) {
      console.log("客户信息:", this.clientid2Name);
      if (this.clientid2Name == row.uRealName) {
        this.$message("客户信息不能重复");
      } else {
        this.$router.push({
          path: "decorateDetails",
          query: { uNick_Name: row.uRealName }
        });
        this.$store.commit("checkNIckName", row.uRealName);
        this.$store.commit("checkNIckNameID", row.uid);
      }
    }
  }
};
