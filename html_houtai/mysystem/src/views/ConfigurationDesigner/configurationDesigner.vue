    ...
    <div class="search_all">
      <div>
        ...
        <div class="filter-container">
          手机号:
          <el-input v-model="formData.uMobile" placeholder="请输入手机号" style="width: 200px;" oninput = "value=value.replace(/[^\d]/g,'')" class="filter-item" />
        </div> 
        <div class="filter-container">
          姓名:
        ...
        <div class="filter-container">
          <el-button style="float:right;margin-left:1rem;margin-right:1rem;" @click="$router.back(-1)" plain>上一页</el-button>
      ...
      <div class="search_listOne">
          <div class="filter-container">
            项目数量:
            <el-input-number v-model="formData.projectCountMin" controls-position="right" @change="handleChange" :min="0" :max="Infinity"></el-input-number>
          </div>
          <div class="filter-container">
            ——
      ...
      <template slot-scope="scope">
        <el-button
          size="mini"
           type="primary"
           plain
          @click="celent_more(scope.$index, scope.row)">加入项目</el-button>
      </template> 
    </el-table-column>
  </el-table>
  <!-- 分页 -->
...
import {designerList} from '../../config/api'
export default {
    name: 'configurationDesigner',
    data() {
      return {
        pageTotal: 1,
        showRow: false,
    ...
    methods: {
      initDataList() {
        ...
        const addCelentUserList = designerList(CurrentList)
        addCelentUserList.then(res => {
          console.log('res', res)
      ...
      celent_more(index, row) {
         this.$router.push({ path: 'decorateDetails', query: {uNick_Name: row.uRealName}})
         this.$store.commit('addDesigner',row.uRealName);
         this.$store.commit('addDesignerID',row.uid); 
